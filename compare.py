...
class Compare:
    ...
    def __compare_two(name, content, std, grader):
        ...
        if not result:
            name1=''.join(re.findall(r'(.+).exe',name))
            nameout =''.join(re.findall(r'read (.+),',str(info)))
            namestd =''.join(re.findall(r'expected (.+).',str(info)))
            with open("%s.out"%name1, "w") as f:
                f.write(f"{nameout}")
            with open("%sstd.out"%name1, "w") as f:
                f.write(f"{namestd}")
